  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      if (newStatus === "confirmed" || newStatus === "cancelled") {
        setEmailMsg((p) => ({
          ...p,
          [apt.id as string]: `Email sent to ${apt.customerEmail}`,
        }));
        setTimeout(
          () =>
            setEmailMsg((p) => {
              const n = { ...p };
              delete n[apt.id as string];
              return n;
            }),
          4000,
        );
    ...
    <div className="p-6 xl:p-8">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`status-badge ${STATUS_COLORS[apt.status as keyof typeof STATUS_COLORS]}`}
                      >
                        {apt.status as string}
                      </span>
                    </div>
                    <p className="font-body text-xs text-slate">
                      {apt.customerEmail as string}
                    </p>
                    {apt.customerPhone ? (
                      <p className="font-body text-xs text-slate">
                        {String(apt.customerPhone)}
                      </p>
                    ) : null}
                    <div className="flex items-center gap-4 mt-3 flex-wrap">
                      <span className="flex items-center gap-1.5 font-mono text-[10px] tracking-wider text-charcoal">
                        <Calendar size={11} className="text-champagne" />
                        {apt.date as string}
                      </span>
                      <span className="flex items-center gap-1.5 font-mono text-[10px] tracking-wider text-charcoal">
                      ...
                      <span className="flex items-center gap-1.5 font-mono text-[10px] tracking-wider text-charcoal">
                        <User size={11} className="text-champagne" />
                        {apt.designerName as string}
                      </span>
                    </div>
                    <div className="mt-2">
                      <span className="font-mono text-[10px] uppercase tracking-wider bg-sand/20 text-charcoal px-2 py-0.5">
                        {
                          APPOINTMENT_TYPE_LABELS[
                            apt.type as keyof typeof APPOINTMENT_TYPE_LABELS
                          ]
                        }
                      </span>
                    </div>
                    {apt.notes ? (
                      <div className="flex items-start gap-1.5 mt-3 max-w-sm">
                        <MessageSquare
                          size={11}
                          className="text-slate mt-0.5 flex-shrink-0"
                        />
                ...
                <div className="flex flex-col gap-2 sm:items-end flex-shrink-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {emailMsg[apt.id as string] && (
                      <span className="font-mono text-[10px] text-green-600 animate-fade-in">
                        ✓ {emailMsg[apt.id as string]}
                      </span>
                    )}
                  </div>
          ...
          {filtered.length === 0 && (
            <div className="admin-card text-center py-12">
              <Calendar
                size={32}
                className="mx-auto text-slate mb-3"
                strokeWidth={1}
              />
        ...
        <div className="space-y-6">
          {Array.from(new Set(filtered.map((a) => a.date as string)))
            .sort()
            .map((date) => (
              <div key={date}>
                  ...
                  {filtered
                    ...
                    .map((apt) => (
                      <div
                        ...
                        className="admin-card border-l-4 border-l-champagne"
                      >
                        ...
                        <p className="font-mono text-[10px] text-slate tracking-wider mb-1">
                          {apt.time as string} · {apt.designerName as string}
                        </p>
                        <span className="font-mono text-[10px] uppercase tracking-wider bg-sand/20 text-charcoal px-2 py-0.5">
                          {
                            APPOINTMENT_TYPE_LABELS[
