  ...
  return (
    <div className="p-6 xl:p-8">
      <div className="flex items-start justify-between mb-8">
        ...
        <Link href="/admin/outfits/new" className="btn-primary">
        ...
        <div className="admin-card overflow-hidden p-0">
          <div className="flex gap-4 px-4 py-3 border-b border-[#EDE8DF] text-xs">
            ...
            <span className="font-mono text-green-600">
              {outfits.filter((o) => o.inStock).length} in stock
            </span>
            <span className="font-mono text-red-500">
              {outfits.filter((o) => !o.inStock).length} sold out
            </span>
          </div>
              ...
              <thead>
                <tr className="border-b border-[#EDE8DF]">
                  {[
                    ...
                    <th
                      key={h}
                      className={`text-left px-4 py-3 font-mono text-[10px] tracking-widest uppercase text-slate ${["Designer", "Category", "Status"].includes(h) ? "hidden md:table-cell" : ""}`}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {filtered.map((outfit) => (
                  <tr
                    ...
                    className="border-b border-[#EDE8DF] last:border-0 hover:bg-[#FAF7F2] transition-colors"
                  >
                    ...
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-body text-sm">
                          {formatPrice(outfit.price as number)}
                        </p>
                        {outfit.originalPrice ? (
                          <p className="font-body text-xs text-slate line-through">
                            {formatPrice(Number(outfit.originalPrice))}
                          </p>
                        ) : null}
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <div className="flex flex-col gap-1">
                        <span
                          className={`status-badge ${outfit.inStock ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
                        >
                          {outfit.inStock ? "In Stock" : "Sold Out"}
                        </span>
                        {outfit.featured ? (
                          <span className="status-badge bg-champagne/20 text-champagne">
                            Featured
                          </span>
                        ) : null}
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() =>
                            handleToggle(
                              outfit.id as string,
                              "inStock",
                              outfit.inStock as boolean,
                            )
                          }
                          className="p-1.5 text-slate hover:text-obsidian transition-colors"
                          title={
                            outfit.inStock ? "Mark sold out" : "Mark in stock"
                          }
                        >
                          {outfit.inStock ? (
                            <Eye size={15} />
                        ...
                        <button
                          onClick={() => setDeleteConfirm(outfit.id as string)}
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12 text-slate">
                ...
                <p className="font-body text-sm">
                  ...
                  <Link href="/admin/outfits/new" className="underline">
                    add a new outfit
                  </Link>
                  .
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-obsidian/40 backdrop-blur-sm p-4">
          <div className="bg-white max-w-sm w-full p-6 shadow-2xl animate-fade-up">
            ...
            <p className="font-body text-sm text-slate mb-6">
              This cannot be undone. The outfit will be permanently removed.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2 bg-red-500 text-white font-body text-xs tracking-widest uppercase hover:bg-red-600 transition-colors"
              >
                Delete
              </button>
            </div>
